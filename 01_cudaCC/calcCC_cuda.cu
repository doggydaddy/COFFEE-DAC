...
void getNthSeries(float *data, float *output, int n, int gV, int nt) 
{
    int c;
    int i;
...
int main ( int argc , char * argv[] ) 
{
    ...
    unsigned long gV = getGoodValues(mask, nx_data, ny_data, nz_data);
	printf("number of voxels within the mask: %ld\n", gV);
	...
	if ( (input_size*3)+(output_size*2) > (system_memory*0.95) ) 
	{
	...
	float *cpu_ccmat = (float *)malloc(output_size); // this is just for testing
	float *gpu_ccmat = (float *)malloc(output_size);
	if ( h_data == NULL || cpu_ccmat == NULL || gpu_ccmat == NULL ) 
	{
	...
	printf("Copy input data from the host memory to the CUDA device (%zu bytes)\n", input_size);
	...
	printf("Copy output data from the device back to host memory (%zu bytes)\n", output_size);
    ...
    /*
     * Calculating the same thing on the CPU with OMP.
     * This is just for performance testing and should
     * be commented away when in actual use.
     */
	printf("Calculating the same thing on the CPU ... \n");
	double start_cpu;
	double end_cpu;
    ...
    for ( int i=0; i<gV; ++i ) 
	{
        for ( int j=i+1; j<gV; ++j ) 
		{
            ...
            cpu_ccmat[k] = calcCrossCorr_omp(h_data, i, j, gV, nt_data);  
        }
    }
    end_cpu = omp_get_wtime();

    int cpu_in_ms = (int)( (end_cpu - start_cpu) * 1000 );
    int cpu_minutes = cpu_in_ms / ( 1000 * 60 );
    int cpu_seconds = ( cpu_in_ms / 1000 ) % 60;
    int cpu_milliseconds = cpu_in_ms % 1000;
	printf("Time taken: %d minutes, %d seconds, and %d milliseconds.\n", cpu_minutes, cpu_seconds, cpu_milliseconds);

	/* 
     * -------------------------------------------------------
	 * Checking the results between cpu and gpu implmentations
	 * -------------------------------------------------------
	 */
	printf("Checking results between CPU and CUDA calculations ....  \n");
	for (int i=0; i<num_elem_output; i++) {
		if ( fabs(cpu_ccmat[i] - gpu_ccmat[i]) > 1e-4 ) {
			printf("Answer checking failed! at index %i, with CPU giving %f and CUDA giving %f\n", i, cpu_ccmat[i], gpu_ccmat[i]);
			exit(EXIT_FAILURE);
		}
	}
	printf(" ... SUCCESS!!! \n ");
